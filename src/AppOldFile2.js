...
function App() {
  ...
  //{userToken: lkajdjafla;fadf} localStorage.TOKEN_STORAGE_ID = lkajdjafla;fadf

  //get initial currentUserToken from localStorage
  const initialToken = localStorage.getItem(TOKEN_STORAGE_ID) || null;
  ...
  useEffect(function loadUserInfoWithToken() {
    ...
    if (currentUserToken) localStorage.setItem(TOKEN_STORAGE_ID, currentUserToken)
    const tokenFromLS = localStorage.getItem(TOKEN_STORAGE_ID);
   
    async function getUserFromToken() {
      console.debug("App useEffect getUserFromToken",
        "currentUserToken=", currentUserToken);
      if (tokenFromLS){
        try {
  ...
  async function login(formData) {
    ...
    localStorage.setItem(TOKEN_STORAGE_ID, token); //user: kldjldjaldkfja
